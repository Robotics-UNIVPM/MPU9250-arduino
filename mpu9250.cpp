...
void MPU9250::updateGyr() {
  ...
  gyr[2] = ((int16_t)rawData[4] << 8) | rawData[5] ;
}

void MPU9250::updateAll() {
  uint8_t rawData[14]; //per leggere 14 registri consecutivi
  readBytes(ACCEL_XOUT_H, 14, rawData);
  acc[0] = ((int16_t)rawData[0] << 8) | rawData[1] ;
  acc[1] = ((int16_t)rawData[2] << 8) | rawData[3] ;
  acc[2] = ((int16_t)rawData[4] << 8) | rawData[5] ;
...
uint8_t MPU9250::readByte(uint8_t subAddress) {
  ...
  Wire.requestFrom((uint8_t)MPU9250_ADDRESS, (uint8_t) 1);
  data = Wire.read();
  return data;
}

void MPU9250::readBytes(uint8_t subAddress, uint8_t count, uint8_t * dest) {
  Wire.beginTransmission(MPU9250_ADDRESS);
  Wire.write(subAddress);
  Wire.endTransmission(false);
  uint8_t i = 0;
  Wire.requestFrom((uint8_t)MPU9250_ADDRESS, count);
...
void MPU9250::getAres() {
  ...
  void MPU9250::calibrate(float * gyroBias, float * accelBias)
  {
